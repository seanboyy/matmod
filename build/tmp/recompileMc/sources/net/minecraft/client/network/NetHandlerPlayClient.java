...
public class NetHandlerPlayClient implements INetHandlerPlayClient
{
    ...
    public void handleSpawnObject(SPacketSpawnObject packetIn)
    {
        ...
        {
            entity = EntityMinecart.func_184263_a(this.clientWorldController, d0, d1, d2, EntityMinecart.Type.getById(packetIn.getData()));
        }
        ...
        if (entity != null)
        {
            ...
            if (packetIn.getData() > 0)
            {
                ...
                {
                    Entity entity2 = this.clientWorldController.getEntityByID(packetIn.getData() - 1);
    ...
    public void handleEntityTeleport(SPacketEntityTeleport packetIn)
    {
        ...
        if (entity != null)
        {
            ...
            if (!entity.canPassengerSteer())
            {
                ...
                {
                    entity.setPositionAndRotation2(entity.posX, entity.posY, entity.posZ, f, f1, 0, true);
                }
                else
                {
    ...
    public void handlePlayerPosLook(SPacketPlayerPosLook packetIn)
    {
        ...
        this.netManager.sendPacket(new CPacketPlayer.C06PacketPlayerPosLook(entityplayer.posX, entityplayer.getEntityBoundingBox().minY, entityplayer.posZ, entityplayer.rotationYaw, entityplayer.rotationPitch, false));
    ...
    public void handleChunkData(SPacketChunkData packetIn)
    {
        ...
        chunk.fillChunk(packetIn.func_186946_a(), packetIn.getExtractedSize(), packetIn.func_149274_i());
    ...
    public void handleConfirmTransaction(SPacketConfirmTransaction packetIn)
    {
        ...
        {
            this.addToSendQueue(new CPacketConfirmTransaction(packetIn.getWindowId(), packetIn.getActionNumber(), true));
        }
    }
    ...
    public void handleUpdateSign(SPacketUpdateSign packetIn)
    {
        ...
        {
            logger.debug("Unable to locate sign at " + packetIn.getPos().getX() + ", " + packetIn.getPos().getY() + ", " + packetIn.getPos().getZ());
        }
    }
    ...
    public void handleUpdateTileEntity(SPacketUpdateTileEntity packetIn)
    {
        ...
        if (this.gameController.theWorld.isBlockLoaded(packetIn.getPos()))
        {
            ...
            {
                ((GuiCommandBlock)this.gameController.currentScreen).func_184075_a();
            }
        }
    }
    ...
    public void handleEntityEquipment(SPacketEntityEquipment packetIn)
    {
        ...
        if (entity != null)
        {
            entity.setItemStackToSlot(packetIn.func_186969_c(), packetIn.getItemStack());
        }
    }
    ...
    public void handleChangeGameState(SPacketChangeGameState packetIn)
    {
        ...
        else if (i == 4)
        {
            if (j == 0)
            {
                this.gameController.thePlayer.sendQueue.addToSendQueue(new CPacketClientStatus(CPacketClientStatus.State.PERFORM_RESPAWN));
                this.gameController.displayGuiScreen(new GuiDownloadTerrain(this));
            }
            else if (j == 1)
            {
    ...
    public void handleCombatEvent(SPacketCombatEvent packetIn)
    {
        ...
        {
            Entity entity = this.clientWorldController.getEntityByID(packetIn.field_179774_b);
        ...
        if (this.checkResourcePackURL(s, s1))
        {
            if (s.startsWith("level://"))
            {
                ...
                if (file2.isFile())
                {
                    ...
                    Futures.addCallback(this.gameController.getResourcePackRepository().setResourcePackInstance(file2), this.createResourcePackStatusCallback(s1));
                }
                else
                {
            ...
            else
            {
                ...
                {
                    this.netManager.sendPacket(new CPacketResourcePackStatus(s1, CPacketResourcePackStatus.Action.ACCEPTED));
                    Futures.addCallback(this.gameController.getResourcePackRepository().downloadResourcePack(s, s1), this.createResourcePackStatusCallback(s1));
                }
                            ...
                            NetHandlerPlayClient.this.gameController.displayGuiScreen(new GuiYesNo(new GuiYesNoCallback()
                            {
                                public void confirmClicked(boolean result, int id)
                                {
                                    ...
                                    if (result)
                                    {
                                        ...
                                        Futures.addCallback(NetHandlerPlayClient.this.gameController.getResourcePackRepository().downloadResourcePack(s, s1), NetHandlerPlayClient.this.createResourcePackStatusCallback(s1));
                                    }
                                    else
                                    {
                                        if (NetHandlerPlayClient.this.gameController.getCurrentServerData() != null)
    ...
    private boolean checkResourcePackURL(String url, String hash)
    {
        ...
        {
            this.netManager.sendPacket(new CPacketResourcePackStatus(hash, CPacketResourcePackStatus.Action.FAILED_DOWNLOAD));
            return false;
        }
    }

    private FutureCallback<Object> createResourcePackStatusCallback(final String hash)
    {
        return new FutureCallback<Object>()
        {
            public void onSuccess(Object p_onSuccess_1_)
            {
                NetHandlerPlayClient.this.netManager.sendPacket(new CPacketResourcePackStatus(hash, CPacketResourcePackStatus.Action.SUCCESSFULLY_LOADED));
            }
            public void onFailure(Throwable p_onFailure_1_)
            {
                NetHandlerPlayClient.this.netManager.sendPacket(new CPacketResourcePackStatus(hash, CPacketResourcePackStatus.Action.FAILED_DOWNLOAD));
            }
        };
    }
    ...
    public void handleCustomPayload(SPacketCustomPayload packetIn)
    {
        ...
        else if ("MC|DebugPath".equals(packetIn.getChannelName()))
        {
            ...
            this.gameController.debugRenderer.debugRendererPathfinding.func_188289_a(j, pathentity, f);
        }
    }
